...
from robotnik.app import RobotnikApp
from robotnik.config import Config
...
DEFAULT_CONFIG_PATH = join(ROOT_PATH, 'robotnik', 'local.conf')
...
def main(args=None, main_loop=None, app=RobotnikApp, server_impl=HTTPServer, get_ioloop=get_ioloop):
    if args is None:
        args = sys.argv[1:]

    parser = argparse.ArgumentParser()
    parser.add_argument('--port', '-p', type=int, default="7000", help="Port to start the server with.")
    ...
    logging.info('-- Robotnik started listening in %s:%d --' % (options.bind, options.port))
    try:
        main_loop.start()
    except KeyboardInterrupt:
        logging.info('')
        logging.info('-- Robotnik closed by user interruption --')
